...
def predict_tta(model, loader):
    ...
    with torch.no_grad():
        for img, lbl in loader:
            ...
            # Default Argmax prediction (before optimization)
...
def plot_confusion_matrix(y_true, y_pred, title, filename):
    ...
    # Calculate row-wise percentages (Recall)
...
def plot_roc_curves(y_true, y_probs, title, filename):
    ...
    for i in range(5):
        if np.sum(y_bin[:, i]) > 0:
            ...
            roc_auc = auc(fpr, tpr)
            lw = 4.5 if i == 0 else 3.0 # Bold for No DR
...
def main():
    ...
    if os.path.exists('dataset/train.csv'):
        ...
        # Using the same split logic to get validation set
        ...
        y_pred_opt = rounder.predict(y_scores, rounder.coefficients())
        ...
        plot_confusion_matrix(y_true, y_pred_opt, "APTOS 2019 Confusion Matrix", "Figure4_ConfusionMatrix.png")
        plot_roc_curves(y_true, y_probs, "APTOS 2019 ROC Curves", "Figure5_Internal_ROC.png")
    ...
    if os.path.exists('idrid_dataset/idrid_labels.csv'):
        ...
        idrid_loader = DataLoader(RetinopathyDataset(df_idrid, 'idrid_dataset', transform=val_aug), 
                                  batch_size=16, shuffle=False, num_workers=2)
        ...
        plot_roc_curves(y_true_i, y_probs_i, "IDRiD External Validation ROC", "Figure6_External_ROC.png")
